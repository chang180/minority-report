...
|------|------|
| `label` | 顯示名稱 |
| `api_url` | 必填；需為 OpenAI-compatible base URL（不含 `/chat/completions`） |
| `api_key` | encrypted |
| `model` | nullable |
| `provider_options` | nullable JSON；UI 以 textarea 輸入；runtime 合併至 Laravel AI request body。支援保留欄位 `ai_provider_key`（見下方說明） |
| `enabled` | boolean |

#### `provider_options.ai_provider_key`（保留欄位）

自訂 provider 預設使用 OpenAI driver（Responses API `/responses`）。若第三方服務僅支援 **Chat Completions API**（`/chat/completions`），需在 `provider_options` 指定 driver：

```json
{ "ai_provider_key": "deepseek" }
```
...
| 值 | Driver | 端點 | 適用服務 |
|---|---|---|---|
| `openai`（預設）| OpenAI Responses API | `/responses` | OpenAI、MiniMax 等支援 Responses API 者 |
| `deepseek` | DeepSeek Chat Completions | `/chat/completions` | DeepSeek API |

`ai_provider_key` 由 runtime 讀取後移除，**不會**傳送至 LLM request body。
...
**注意**：Laravel AI 的 OpenAI driver 使用 Responses API（`/responses`），非所有第三方服務支援。若服務僅支援 Chat Completions API，需透過 `provider_options.ai_provider_key` 指定對應 driver（見 [§5.3](#53-自訂-provider--user_custom_providers)）。

**`AiTextProviderFactory` URL 行為**：若 `config/ai.php` 中 provider 的 `url` 為空字串（例如 `LOCAL_AI_API_URL=` 留空），runtime 會自動忽略該空 URL，使用 SDK 預設端點，避免 "scheme is not allowed" 錯誤。
