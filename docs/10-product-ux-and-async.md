...
| `running` | 「分析中…」+ spinner |
| `completed` | 既有 consensus / trust / minority UI |
| `failed` | 「處理失敗」+ 錯誤摘要 |
...
- [ ] Migration `processing_status` + backfill
- [ ] `RunAuthenticatedVerificationJob`（含 M8-B grounding 邏輯）
- [ ] Async `POST /verifications` + status JSON + Show polling
- [ ] `GET /verifications` Index + nav
- [ ] Fortify email verification + 本機 auto-verify
- [ ] `verified` middleware on verification routes
- [ ] Replay route + UI + policy
- [ ] `M8A*` tests；全 suite 綠
- [ ] **MUST NOT** 改 Grounding / Trust / Cases
