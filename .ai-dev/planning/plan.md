...
└── 07-milestones.md
└── 08-ui-auth-providers.md   ← M7 Post-MVP（2026-06-14 新增）
...
### M7 Gate 摘要（Post-MVP · spec: docs/08）

| Gate | 交付物 |
|------|--------|
| M7-A | Fortify + vue-starter-kit 移植 + Welcome + Demo 路由 |
| M7-B | Provider 設定 + Admin demo + Dashboard + auth verification |

**M7 開工前 MUST** 完成 `docs/08-ui-auth-providers.md` 與 `07` §M7（spec-driven）。

### M8 Gate 摘要（規劃中 · 詳見 [m8-roadmap.md](../.ai-dev/planning/m8-roadmap.md)）

| Gate | 交付物 |
|------|--------|
| M8-A | Verification 列表、async Job、UX 完成度 |
| M8-B | Grounding v1 + trust cap（需 `docs/09`） |
| M8-C | Semantic claim alignment |

**M8-B 開工前 MUST** 完成 `docs/09-grounding-and-trust.md`。
